...
class DeviceDefinition(PicDevice):
    ...
    DEVICE_REV_ADDRESS_B = 0x1000A # Word address: 0x8005
    ...
    VDDMIN = 2.3       # Lowest operating voltage
    VDDMAX = 5.5       # Highest operating voltage
    VBODMAX = 2.10     # Maximum value of the lowest possible BOD
    VBODHIGH = 2.85    # Maximum value of the highest possible BOD
    ...
    def read_device_revision(self):
        ...
        # Step to device ID address
        # PIC16 devices uses word adressing so only one increment per word please
        for _ in range((self.DEVICE_REV_ADDRESS_B - self.CONFIG_MEMORY_ADDRESS_B) // 2):
            self.prog.command(self.INCREMENT_ADDRESS)

        # Update address pointer
        self.last_address_b = self.DEVICE_REV_ADDRESS_B
