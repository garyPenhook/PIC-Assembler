...
class PythonScriptedPicDebugger(CmsisAtiPicDebugger):
    ...
    def _check_operating_voltage(self):
        ...
        # Tolerance is 5mV
        VDD_TOLERANCE = 0.005
        self.logger.info("Operating voltage: %.01fV", self.voltage)
    ...
    def _check_vbodmax_voltage(self):
        """
        Checks operating voltage against maximum lowest BOD voltage (VBODMAX) of the device and
        gives appropriate messages to the user
        """
        self.logger.info("Operating voltage: %.01fV", self.voltage)

        # Check operating voltage against VBODMAX in device file
        if self.voltage < self.device_model.VBODMAX:
            msg = "The target voltage ({0:.02f}V) is below VBODMAX for the {1:s} ({2:.01f}V)".format(
                self.voltage, self.device_model.DISPLAY_NAME, self.device_model.VBODMAX)
            msg += "\r\nWhen erasing or programming flash memory on the {0:s} ".format(self.device_model.DISPLAY_NAME)
            msg += "the operating voltage must be above the active BOD threshold (if BOD is enabled), "
            msg += "or the highest BOD threshold for the lowest BOD voltage"
            msg += "(VBODMAX={0:.01f}V)".format(self.device_model.VBODMAX)
            msg += "\r\nRaise the target voltage accordingly and try again."
            self.logger.error(msg)
            raise VoltageException(msg)

    def _check_plausible_bod_voltage_error(self):
        """
        ...
        # Check operating voltages against VBODHIGH in device file
        if self.voltage < self.device_model.VBODHIGH:
            msg += "\r\nOne possible cause of this may be operating at a voltage either "
            msg += "below the active BOD level or VBODMAX. "
            msg += "The target voltage ({0:.02f}V) is below ".format(self.voltage)
            if self.voltage < self.device_model.VBODMAX:
                msg += "VBODMAX for the {0:s} ({1:.01f}V)".format(self.device_model.DISPLAY_NAME,
                                                                  self.device_model.VBODMAX)
                msg += "\r\nRaise the target voltage accordingly and try again."
            else:
                msg += "the maximum BOD value for the {0:s} ({1:.01f}V)".format(self.device_model.DISPLAY_NAME,
                                                                                self.device_model.VBODHIGH)
                msg += "\r\nIf the BOD is enabled, raise the target voltage accordingly and try again."
    ...
    def enter_tmod(self):
        ...
        if device_id == 0x0000 or device_id == 0xFFFF:
            ...
            # Voltage check with respect to BOD
            self._check_plausible_bod_voltage_error()
    ...
    def read_config_memory(self, byte_address, numbytes):
        ...
        if hasattr(self.device_model, 'read_config_byte'):
            ...
            while numbytes > 0:
                self.logger.info("Read config byte at 0x{:02X}".format(byte_address))
            ...
            # Loop until done
            while words > 0:
                self.logger.info("Read config word at 0x{:02X}".format(byte_address))
    ...
    def write_config_memory(self, byte_address, data):
        ...
        if hasattr(self.device_model, 'write_config_byte'):
            ...
            for i in range(len(data)):
                values = [data[i]]
                self.logger.info("Writing one config byte at 0x{:02X}, value: ".format(byte_address + i, values[0]))
...
class PythonScriptedPic16Debugger(PythonScriptedPicDebugger):
    ...
    def debug_get_hardware_stack(self):
        ...
        :rtype: dict
        """
        self.logger.info("Get Hardware Stack")
        stack_info = {}
