...
class DeviceDefinition(PicDevice):
    ...
    FLASH_WRITE_BYTES_PER_PAGE = 256
    ...
    VDDMIN = 1.8        # Lowest operating voltage
    VDDMAX = 5.5        # Highest operating voltage
    VBORMAX = 2.1       # Maximum value of the lowest possible BOR
    VBORHIGH = 3.0      # Maximum value of the highest possible BOR

    # Only the config byte addresses in this list are allowed to be written (i.e. blocking access to SAFLOCK and ICSPDIS related config bytes)
    CONFIG_BYTES_WRITABLE = [
        0x300000, 0x300001, 0x300002, 0x300003, 0x300004, 0x300005, 0x300006, 0x300008, 0x300009, 0x30000A
    ]
    ...
    def exit_tmod(self):
        # Tristate clock and data lines from the debugger
    ...
    def read_device_revision(self):
        ...
        # Set address to configuration space
        self.prog.command(self.LOAD_PC_COMMAND)
        self.prog.payload(self.DEVICE_REV_ADDRESS_B)
        # Read a word from NVM
        self.prog.command(self.READ_DATA_NVM_COMMAND)
    ...
    def write_config_byte(self, byte_address):
        """
        Writes one byte to the config space

        :param byte_address: byte address to write
        :return:
        """
        if byte_address not in self.CONFIG_BYTES_WRITABLE:
            self.logger.warning("Writing to config byte address 0x%08X - BLOCKED", byte_address)
            return
    ...
    def read_config_byte(self, byte_address):
        """
        Reads one config byte from config space.

        :param byte_address: address of the byte
        :return:
        """
        # Set the address
        self.prog.command(self.LOAD_PC_COMMAND)
