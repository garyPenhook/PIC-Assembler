...
class ProgExecInterfacePic24(object):
    ...
    def __init__(self):
        ...
        self.hw = None
        self.board = None
        # dsPIC requires a 25ms delay between PE commands
        # Just use this as default for all 16-bit PICs
        self.inter_command_delay_ms = 25
    ...
    def _sanity_check_proxy_command(self):
        ...
        """
        self.board.delay_ms(self.inter_command_delay_ms)
    ...
    def _write_flash_page_proxy_command(self, byte_address, numbytes):
        ...
        :param numbytes: Number of bytes to write (including the upper byte of each instruction word that
    ...
    def _check_pe_response(self, cmd, databytes, response):
        ...
        if status == self.PE_STATUS_PASS:
            ...
            raise PrimitiveException("PE command failed, response status: FAIL, {}".format(error_msg))
        elif status == self.PE_STATUS_NACK:
            raise PrimitiveException("PE command failed, response status: NACK, {}".format(error_msg))
