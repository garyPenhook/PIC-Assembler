...
class CmsisAtiPicDebugger(object):
    ...
    def setup_session(self, tool, options):
        ...
        if isinstance(tool, ConfigGeneratorTool) or isinstance(tool, EmbeddedTool) or isinstance(tool, PrinterTool):
            # Report the voltage as 3.3V to keep tests/generation happy
            self.voltage = 3.3
            self.logger.info("Operating voltage fixed at: {0:0.2f}V".format(self.voltage))
    ...
    def _initialise_transport(self, tool_or_transport):
        ...
        voltage_mv = hk.get_le16(hk.HOUSEKEEPING_CONTEXT_ANALOG, hk.HOUSEKEEPING_ANALOG_VTREF)
        self.voltage = voltage_mv / 1000.0
        self.logger.info("> Operating voltage: {0:0.2f}V".format(self.voltage))

    # API functions for the config generator in pymcuprog used to generate device config for kits
    # These must be implemented in sub-classes of this class for architectures where they are supported
    ...
    def write_config_word(self, byte_address, data):
        ...
        :type data: bytearray
        """
        self.logger.error("Write config word not implemented")
    ...
    def write_config_byte(self, byte_address, data):
        ...
        :type data: bytearray
        """
        self.logger.error("Write config byte not implemented")
    ...
    def write_flash_page(self, byte_address, data):
        ...
        :type data: bytearray
        """
        self.logger.error("Write flash page not implemented")
    ...
    def write_eeprom_block(self, byte_address, data):
        ...
        :type data: bytearray
        """
        self.logger.error("Write EEPROM block not implemented")
    ...
    def write_user_id_word(self, byte_address, data):
        ...
        :type data: bytearray
        """
        self.logger.error("Write user ID not implemented")
