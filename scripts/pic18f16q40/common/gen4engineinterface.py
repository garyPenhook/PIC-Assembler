...
class Gen4WrapperDebugger(CmsisAtiPicDebugger):
    ...
    DE_TRANSFER_STATUS_OK = 0
    DE_TRANSFER_INVALID_CMD = 0x80
    DE_HANDSHAKE_TIMEOUT_WAIT_CLK_HIGH = 0x81
    DE_HANDSHAKE_TIMEOUT_WAIT_CLK_LOW = 0x82
    DE_TRANSFER_TIMEOUT_WAIT_CLK_HIGH = 0x83
    DE_TRANSFER_TIMEOUT_WAIT_CLK_LOW = 0x84
    DE_TRANSFER_TIMEOUT_WAIT_DATA_LOW = 0x85
    DE_TRANSFER_ERROR = 0x86
    ...
    def setup_session(self, tool, options):
        ...
        if isinstance(tool, PrinterTool):
            ...
            if self.device_model.DEBUGGING_INTERFACE is SpitfireInterface:
                self.logger.info("No primitives are used for debugging based on Spitfire module")
            else:
            ...
            if self.transport:
                ...
                # Setting up debugging stack
                if self.device_model.DEBUGGING_INTERFACE is SpitfireInterface:
                    # For devices with Spitfire module no primitive accumulator is used
                    self.logger.debug("Using SpitfireInterface")
                else:
                    ...
                    self.debug_executive_proxy = PrimitiveFunctionAccumulatorExecuter(self.debug_executive_object,
                                                                                self.debug_controller, use_lambda)

                self.prog_executive_model = self.device_model.PROGRAMMING_EXECUTIVE_INTERFACE
                self.prog_executive_object = self.device_model.PROGRAMMING_EXECUTIVE_INTERFACE()

                self.logger.debug("Creating PE proxy")
                if self.device_model.DEBUGGING_INTERFACE is SpitfireInterface:
                    self.logger.debug("Creating PE primitive controller")
                    self.prog_executive_controller = PrimitiveController(self.transport)
                    self.prog_executive_proxy = PrimitiveFunctionAccumulatorExecuter(self.prog_executive_object,
                                                                                     self.prog_executive_controller)
                else:
                    self.logger.debug("Reusing debug controller")
                    # For the programming executive we use primitives like for the debug executive
                    # so we can just reuse the debug_controller
                    self.prog_executive_proxy = PrimitiveFunctionAccumulatorExecuter(self.prog_executive_object,
    ...
    def enter_tmod_pe(self):
        ...
        :return Programming Executive version
            (one byte, upper nibble is major version and lower nibble is minor version)
        """
        # Already in?
        if self._in_tmod_pe:
        ...
        try:
            self._current_pe_version = self.prog_executive_object.check_pe_connection_by_proxy(
                self.prog_executive_proxy)
        except:
            # Even if a PE is present in the target it can suddenly become unavailable for example if the
            # Debug config bit has just been set
            # If reading the PE version fails normal ICSP should be used as a fallback
            # Disable future PE use (in the current session)
            self.use_pe = False
            self.logger.info("Could not contact PE, falling back to normal ICSP")
            self.enter_tmod()
            return None
    ...
    def read_device_revision(self):
        ...
        result = self.device_proxy.invoke_read(bytes_to_read=4, method=self.device_model.read_device_revision)
        device_rev = binary.unpack_le32(result) >> 16
    ...
    def _read_flash_block(self, byte_address, numbytes):
        ...
        if self.use_pe:
            ...
            data = self.prog_executive_object.read_flash_by_proxy(self.prog_executive_proxy,
                                                                  int(byte_address), numbytes)
        else:
    ...
    def pad(data, pagesize):
        ...
        :return: padded chunk
        """
        # Avoid messing with the data object directly as it might have unintended side effects
        # (like changing the stored PE or DE)
    ...
    def _write_flash_block(self, byte_address, data):
        ...
        for chunk in self.chunks(data, chunksize):
            if not is_blank(chunk):
                ...
                # MPLAB is supposed to always write complete pages but in the past it has in some situations
                # not done so (MPLABX-4885)
    ...
    def _write_flash_chunk(self, byte_address, data):
        ...
        if self.use_pe:
            # We can't use PE mode for config words even though they are implemented as normal flash.
            # This is because config words might mess up the PE execution for example if clocks
            # are changed or debug is enabled
            # Note greater than (not greater than or equal) because byte_address + bytes to be written
            # will point to first memory location after the memory that is currently about to be written
        ...
        for i in range(len(data)):
            if data[i] != read_back[i]:
                if silent_early_exit:
                    # This path is used when a verify failure has an immediate remedy
                    # (like checking if the PE is there)
    ...
    def erase_de_memory(self, byte_address, numbytes):
        ...
        self.device_proxy.invoke(method=self.device_model.erase_testmem_range, byte_address=int(byte_address),
                                        numbytes=numbytes)
    ...
    def write_pe_memory(self, byte_address, data):
        ...
        # We must always write complete pages for PIC24 so we have to pad the data.
        # This is done by MPLAB for normal program memory
    ...
    def write_config_memory(self, address, data):
        ...
        :param data: data to write
        """
        self.logger.error("Write config memory not implemented")
    ...
    def start_programming_operation(self, program_pe=True):
        ...
        if program_pe and not pe_version_match:
            if self.program_exec_address is None:
                self.logger.info("Program Executive not set. Using normal ICSP mode")
            else:
                self.enter_tmod()
                self.logger.info("Checking PE")
                status = True
                if self._verify_flash_block(self.program_exec_address, self.program_exec_data, silent_early_exit=True):
    ...
    def init_debug_session(self, program_de=True):
        ...
        # If unable to communicate with the DE an exception will be raised
        # and will have to be handled further up the stack.
    ...
    def end_debug_session(self):
        ...
        End debug session
        """
        self.logger.error("End debug session not implemented")
    ...
    def _check_de_response(self, result):
        ...
        """
        cmd = result[0]
        if cmd in (self.DE_COMMAND, self.DE_COMMAND_DATA_ONLY):
            self.logger.info("DE response: %X %X %X %X", result[0], result[1], result[2], result[3])
            status = result[1]
            if status == self.DE_TRANSFER_INVALID_CMD:
                raise Exception("Invalid command received by DE, command ignored")
            if status == self.DE_HANDSHAKE_TIMEOUT_WAIT_CLK_HIGH:
                raise Exception("Timeout waiting for DE handshake to signal clock high")
            if status == self.DE_HANDSHAKE_TIMEOUT_WAIT_CLK_LOW:
                raise Exception("Timeout waiting for DE handshake to signal clock low")
            if status == self.DE_TRANSFER_TIMEOUT_WAIT_CLK_HIGH:
                raise Exception("Timeout waiting for DE transfer to signal clock high")
            if status == self.DE_TRANSFER_TIMEOUT_WAIT_CLK_LOW:
                raise Exception("Timeout waiting for DE transfer to signal clock low")
            if status == self.DE_TRANSFER_TIMEOUT_WAIT_DATA_LOW:
                raise Exception("Timeout waiting for DE transfer to signal data low")
            if status == self.DE_TRANSFER_ERROR:
                raise Exception("DE communication failed, unknown error")
    ...
    def debug_erase(self, address):
        ...
        :return:
        """
        self.logger.error("Debug erase not implmented")
    ...
    def set_pc(self, pc):
        ...
        :param pc: PC value
        """
        self.logger.error("Set PC not implemented")
    ...
    def release_from_reset(self):
        ...
        # Run exit_tmod to make sure the device is not in tmod because if the target
        # is in tmod the MCLR pin will already be high and release_from_reset will
        # have no effect, the target will just be kept in tmod/progmode and code
        # won't start to run
        self.exit_tmod()
    ...
    def debug_write_flash(self, byte_address, data):
        ...
        :param data: data to write
        """
        self.logger.error("Debug write flash not implemented")
    ...
    def set_sw_bp(self, byte_address, instruction, flags):
        ...
        self.logger.error("Set SW breakpoint not implemented")
