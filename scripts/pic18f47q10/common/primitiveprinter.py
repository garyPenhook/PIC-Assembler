...
class PrimitiveResourceProviderPrinter(PrimitiveResourceProvider):
    ...
    def programming_interface(self, variant):
        if variant == ProgInterfaceIcspC8D24:
            ...
            raise Exception("Invalid ICSP variant requested (%s)." % (variant))
...
class PrimitiveFunctionXmlGenerator(PrimitiveFunctionAccumulator):
    ...
    def invoke(self, method, bytes_to_read=0, **kwargs):
        ...
        # The returned data is mostly ignored when generating XML but when reading device ID the returned value is
        # checked agains all FFs or all 0es. This is not a problem when just generating XML but it is annoying since it
        # will trigger some error logging. To avoid this the dummy data value is set to 0xDD.
        result = bytearray([0xDD]*bytes_to_read)
        return result
