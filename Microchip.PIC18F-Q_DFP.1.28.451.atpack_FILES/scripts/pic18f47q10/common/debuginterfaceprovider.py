...
class DebugExecApiPic24Vx(PicDebugInterface):
    ...
    def _de_command_3_2(param1, param2):
        ...
        """
        return bytearray([param1 & 0xFF, (param1 >> 8) & 0xFF, (param1 >> 16) & 0xFF,
    ...
    def _de_command_4_2(param1, param2):
        ...
        """
        return bytearray([param1 & 0xFF, (param1 >> 8) & 0xFF, (param1 >> 16) & 0xFF,
                         (param1 >> 24) & 0xFF, param2 & 0xFF, (param2 >> 8) & 0xFF])
...
class DebugExecApiPic18Vx(PicDebugInterface):
    ...
    DE_COMMAND_GET_HARDWARE_STACK = 0x12
    ...
    @staticmethod
    def _de_command_2_3bytes_2bytes(param1, param2):
        """
        Header generator for a two parameter command, first is a 3 byte parameter and second is a 2 byte parameter
        """
        return bytearray([param1 & 0xFF, (param1 >> 8) & 0xFF, (param1 >> 16) & 0xFF, param2 & 0xFF, (param2 >> 8) & 0xFF])

    @staticmethod
    def _de_command_1_3bytes(param):
        """
        Header generator for a single parameter with 3 bytes command
        """
        return bytearray([param & 0xFF, (param >> 8) & 0xFF, (param >> 16) & 0xFF])

    def de_command_set_pc(self, pc):
        """
        Header generator for a set PC command
        """
        return self._de_command_1_3bytes(pc)

    def de_command_erase(self, address):
        """
        Header generator for an erase command
        """
        return self._de_command_1_3bytes(address)

    def de_command_memory_write(self, address, length):
        """
        Header generator for a memory (not NVM) write command
        """
        return self._de_command_2(address, length)

    def de_command_flash_write(self, address, length):
        """
        Header generator for a flash write command
        """
        return self._de_command_2_3bytes_2bytes(address, length)

    def de_command_memory_read(self, address, length):
        """
        Header generator for a memory (not NVM) read command
        """
        return self._de_command_2(address, length)

    def de_command_flash_read(self, address, length):
        """
        Header generator for a flash read command
        """
        return self._de_command_2_3bytes_2bytes(address, length)
    ...
    def de_get_hardware_stack(self, stack_dump_bytes):
        """
        DE execute, 0 meta-data bytes sent, <architecture-dependent> bytes received
        """
        self.debug_proxy.debug_command(self.DE_COMMAND_GET_HARDWARE_STACK, 0, stack_dump_bytes)

    def debug_erase_page(self, delay):
        """
        DE execute, 3 meta-data bytes sent, 0 bytes received
        Meta-data bytes: addrL, addrH, addrU
        """
        self.debug_proxy.debug_command(self.DE_COMMAND_ERASE_PAGE, 3, 0)
        self.board.delay_us(delay)

    def debug_read_flash(self, numbytes):
        """
        DE execute, 5 meta-data bytes sent, n bytes received)
        Meta-data bytes: addrL, addrH, addrU, wordsL, wordsH
        """
        self.debug_proxy.debug_command(self.DE_COMMAND_READ_FLASH, 5, numbytes)
    ...
    def debug_write_flash(self, numbytes, delay):
        ...
        Meta-data bytes: addrL, addrH, addrU, wordsL, wordsH
        """
        self.debug_proxy.debug_command(self.DE_COMMAND_WRITE_FLASH, numbytes + 5, 0)
