...
def process_primitive_sequence(cmd, use_lambda=True):
    """
    ...
    - Wrap as a lambda function if use_lambda is True
    - Flatten and extract tokens (for parameterising)

    :param cmd: Sequence to process
    :type cmd: Nested tree of lists and bytearrays
    :returns: command array and token array
...
def do_flatten_tree(source, result, tokens):
    ...
    if isinstance(source, ParametricValueToken):
        ...
        LOGGER.info("%d byte Value Token called %s at %d (transform %d)", source.bytecount, source.type,
                                                                          source.offset, source.transform)
