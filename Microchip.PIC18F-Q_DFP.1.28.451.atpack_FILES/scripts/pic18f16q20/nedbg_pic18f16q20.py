...
Reference: https://confluence.microchip.com/x/mkp_Cw
"""
import os
import logging
...
from common.spitfireinterface import SpitfireException
...
def start_programming_operation():
    ...
    except VoltageException, e:
        terminal.show_info_dialog_blocking(str(e))
        raise
...
def read_device_id():
    ...
    """
    terminal.display("API command: read ID\n")
    id = debugger.read_id()
    # Read device revision if its present in the device model
    if hasattr(debugger.device_model, 'read_device_revision'):
        rev = debugger.read_device_revision()
    else:
        rev = 0x0000

    if any(x in device_name for x in ["pic24", "dspic33"]):
        # MPLAB expects a 32-bit integer consisting of 2 16-bit words: revision (LS) and ID (MS)
        return (id << 16) | rev
    # MPLAB expects a 32-bit integer consisting of 2 16-bit words: revision (MS) and ID (LS)
    return (rev << 16) | id
...
def copy_data(source, destination):
    ...
    if len(source) > len(destination):
        ...
        terminal.show_info_dialog_blocking("{}".format(message))
...
def prog_read(type_of_mem, address, length, data):
    ...
    if str(type_of_mem) == "Pgm":
        ...
        # User ID/Test memory
        copy_data(debugger.read_flash_memory(byte_address, length), data)
    elif str(type_of_mem) == "DIA" or str(type_of_mem) == "DCI":
        # Device Information Area/Device Configuration Information memory
        copy_data(debugger.read_flash_memory(byte_address, length), data)
    elif str(type_of_mem) == "UDID":
        # Unique Device Identifier
        if "pic24" in device_name or "dspic33" in device_name:
            # Length field may need rounding up for this memtype
            bytes_to_read = length
            # We round to 16 here which is 12-packs of compact format since that is the granularity of the data provider
            if length % 16:
                bytes_to_read += 16-(length % 16)
            # Read out rounded off length
            response = debugger.read_flash_memory(byte_address, bytes_to_read)
            # and crop
            copy_data(response[0:length], data)
        else:
            terminal.display("UDID memory is not supported for this device.")
    else:
        terminal.display("Unknown memtype: {}!\n".format(str(type_of_mem)))
        data = bytearray([])
...
def begin_debug_session():
    ...
    except (PrimitiveException, SpitfireException) as e:
        # Note this error message is the standard message used in MPLAB when failing to communicate with the
        # Debug Executive for PIC24 devices. It should be kept in sync with what is printed for other tools to
        # make it easier for the user to figure out what is wrong. Even though this message might not be accurate
        # for most 8-bit PIC devices (the only 8-bit PIC known to have different debug pins vs. program pins is
        # the PIC18(L)F45K50 family) there are no other known sources of this error so this is anyway the best
        # feedback that can be given to the user.
        message = ("The target device is not ready for debugging. Please check your configuration bit settings and"
                   " program the device before proceeding. The most common causes for this failure are oscillator and/or"
                   " PGC/PGD settings.")
        # This will show the message in a separate dialog box to avoid it being drowned in the Java stack trace
        terminal.show_info_dialog_blocking(message, "Debugging Failed")
        # This will show the message in the normal script output window
        logger.error(message)
        # This will make sure that MPLAB does not continue attempting to debug.
        raise Exception("Unable to communicate with DE (0x{0:08X})".format(e.code))
...
def fill_stack_info(stack):
    """
    Provide contents of the hardware stack
    """
    terminal.display("API command: stack info\n")
    # Read stack info from the DE
    stack_info = debugger.debug_get_hardware_stack()
    # Send it back in the object provided
    stack.levels = stack_info['levels']
    stack.programCounter = stack_info['program_counter']
    stack.stackPointer = stack_info['stack_pointer']
...
def debug_write(mem_type, start, length, data):
    ...
    if str(mem_type) == "Emulation":
        if "pic24" in device_name or "pic18" in device_name or "dspic33" in device_name:
...
def set_hw_bp(number, address):
    ...
    logger.error("Set hardware breakpoint not implemented")
...
def clear_hw_bp(number):
    ...
    logger.error("Clear hardware breakpoint not implemented")
...
def verify_transfer(type_of_mem, address, data, length):
    ...
    logger.error("Verify memory not implemented\n")

def blank_check():
    """
    Blank-check memory
    """
    terminal.display("API command: Blank check\n")
    logger.error("Blank check not implemented\n")
